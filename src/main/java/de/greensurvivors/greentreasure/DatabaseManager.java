...
import de.greensurvivors.corelib.database.IUserDataDao;
import de.greensurvivors.corelib.database.NoSQLConnectionException;
...
public class DatabaseManager {
    ...
    public @NotNull CompletableFuture<Void> setTreasureContents(final @NotNull Ulid treasureId, final @NotNull List<@NotNull ItemStack> contents) {
        ...
        connectionProvider().virtualExecutorService().execute(() -> {
            if (!connectionProvider().hasConnection()) {
                Bukkit.getScheduler().runTask(plugin, () -> resultFuture.completeExceptionally(new NoSQLConnectionException()));
                return;
            }
            ...
            try (final @NotNull Connection connection = connectionProvider().connection();
        ...
        connectionProvider().virtualExecutorService().execute(() -> {
            ...
            final @NotNull String statementStr =
                "INSERT INTO " + PLAYERDATA_TABLE + " (" +
                        TREASURE_ID_KEY + ", " +
                        IUserDataDao.USER_ID + ", " +
                        TREASURE_FIRST_TIMESTAMP_KEY + ", " +
                        TREASURE_LAST_TIMESTAMP_KEY + ", " +
                        TREASURE_CONTENT_KEY + ") " +
                    "VALUES (" +
                        "?, " +
                        "(SELECT " + IUserDataDao.USER_ID + " FROM " + IUserDataDao.USER_TABLE + " WHERE " + IUserDataDao.USER_UUID + " = ?), " +
                        "?, " +
                        "?, " +
            ...
            final @NotNull String statementStr =
                "SELECT p." + TREASURE_LAST_TIMESTAMP_KEY + "," +
                    ...
                    " COALESCE(p." + TREASURE_CONTENT_KEY + ", t." + TREASURE_CONTENT_KEY + ") AS " + TREASURE_CONTENT_KEY +
                        ...
                        " JOIN " + IUserDataDao.USER_TABLE + " AS u" +
                        " ON p." + IUserDataDao.USER_ID + " = u." + IUserDataDao.USER_ID +
                        " LEFT JOIN " + TREASURE_TABLE + " AS t" +
                        " ON p." + TREASURE_ID_KEY + " = t." + TREASURE_ID_KEY +
                        " WHERE p." + TREASURE_ID_KEY + " = ? AND u." + IUserDataDao.USER_UUID + " = ?";
        ...
        connectionProvider().virtualExecutorService().execute(() -> {
            ...
            final String statementStr =
                "DELETE FROM " + PLAYERDATA_TABLE +
                    " WHERE " + TREASURE_ID_KEY + " = ? " +
                    "AND " + IUserDataDao.USER_ID + " = (SELECT " + IUserDataDao.USER_ID + " FROM " + IUserDataDao.USER_TABLE + " WHERE " + IUserDataDao.USER_UUID + " = ?)";
    ...
    public @NotNull CompletableFuture<@NotNull Map<UUID, @NotNull PlayerLootDetail>> getAllPlayerData(final @NotNull Ulid treasureId) {
        ...
        connectionProvider().virtualExecutorService().execute(() -> {
            ...
            final String statementStr =
                "SELECT u." + IUserDataDao.USER_UUID + "," +
                    " t." + TREASURE_FIRST_TIMESTAMP_KEY + "," +
                    " t." + TREASURE_LAST_TIMESTAMP_KEY + "," +
                    " t." + TREASURE_CONTENT_KEY +
                        " FROM " + PLAYERDATA_TABLE + " AS t" +
                        " JOIN " + IUserDataDao.USER_TABLE + " AS u ON t." + IUserDataDao.USER_ID + " = u." + IUserDataDao.USER_ID +
            ...
            try (final Connection connection = connectionProvider().connection();
                ...
                try (final ResultSet resultSet = preparedStatement.executeQuery()) {
                    ...
                    while (resultSet.next()) {
                        final @NotNull UUID playerUUID = UUID.fromString(resultSet.getString(IUserDataDao.USER_UUID));
                        final @NotNull Instant firstLootedInstant = Instant.ofEpochMilli(resultSet.getLong(TREASURE_FIRST_TIMESTAMP_KEY));
                        final @NotNull Instant lastChangedInstant = Instant.ofEpochMilli(resultSet.getLong(TREASURE_LAST_TIMESTAMP_KEY));
    ...
    private void createTablePlayerData() {
        final String statementStr = "CREATE TABLE IF NOT EXISTS " + PLAYERDATA_TABLE + " (" +
            IUserDataDao.USER_ID + " INT UNSIGNED NOT NULL, " +
            ...
            "PRIMARY KEY (" + IUserDataDao.USER_ID + ", " + TREASURE_ID_KEY + "), " +
            "FOREIGN KEY (" + IUserDataDao.USER_ID + ") REFERENCES " + IUserDataDao.USER_TABLE + "(" + IUserDataDao.USER_ID + "), " +
    ...
    // nothing but a handy little shortcut
    private @NotNull IConnectionProvider connectionProvider() {
        return plugin.connectionProvider();
    }
}
